...
> 정리일: **2026-06-10** (30차 갱신) · 정리자: Claude (Fable 5)
> 라이브: https://grandtour-lilac.vercel.app/  · 상세 차수별 기록은 `SESSION_LOG.md`(ground truth) 참조 · 실기기 QA는 `QA_CHECKLIST.md`
> 저장소: https://github.com/rarehotdog/grandtour
> 출발: **2026-06-15** (D−5) · 코드 동결 권장일 경과 → 데이터·실기기 QA 위주
> 푸시 상태: `origin/main`==`a943528`(자간 토큰화까지 푸시 완료). 로컬 **12커밋 미푸시**(25~30차) — **사용자 푸시 보류 지시**, "푸시해줘" 시 푸시. ⚠️ 라이브는 21차 상태 — 컨시어지·기록 경험 포함 25~30차 전부 미배포.
>
> **25~30차 요약(코드)**: 25차 여정 탭 3그룹 → 12 아이콘 그리드·지도 핀 국기 제거·호텔 컴팩트 행(`1e26d04`)+자간 px 토큰화(`a943528`) · 26차 Today 카드 행동 우선 재배치+지금포커스/타임라인 중복 제거(`dd5c9f3`)+`QA_CHECKLIST.md` 신설 · **27차 GT 컨시어지 — 규칙 기반 '지금 필요한 것' 브리핑(홈 최상단, buildConciergeItems 순수 엔진+ConciergeCard)**(`c84ad6d`) · **28차 컨시어지 2단계 — 레디니스 감지(IDB 지도타일·문서금고·환율캐시)+여행 중 urgent 데드라인+자급 날씨**(`c32e23c`) · 29차 가계부 스마트 통화(오늘 나라 통화 자동)+출발일 아침 마지막 레디니스+공항 여유 힌트(`49698f1`) · **30차 영원의 기록 — 저녁 한 줄 캡처(EveningReminder×NoteEditor 인라인)+빈 기록 넛지+기록 캘린더(27칸 도시색)+27일 잔재 정리**(`4cc0041`, 엔진 테스트 46/46).
>
> **21~24차 요약(코드)**: 21차 여정 12서브탭 → 3그룹(`cbcff35`) · 22차 기기 연동 UX + 헤더 배지(`9ac8576`) · 23차 '🧭 오늘의 현지 도구'(during 홈 컨텍스트 집약 + 여정 딥링크) + 비상연락처 오늘 위치 우선(`330439a`) · **24차 네이티브 뒤로가기 풀세트(useBackClose/useScrollLock 훅 — 모달·서브뷰를 하드웨어/제스처 백으로 LIFO 닫기·모달 스크롤 락·여정/기록 서브탭 위치 기억)**(`4d985f7`, history+popstate 단일 sentinel 컨트롤러, iOS standalone 제외).
>
...
| 지표 | 값 |
|---|---|
| **index.html** | 8,764줄 · ~495KB (실제 지리 베이스맵 path 포함) · top-level function 72 |
| **React 컴포넌트** | 64개 top-level + 인라인 (App 포함, 함수형). 27차 신규 `ConciergeCard` |
| **GT 컨시어지(27~30차)** | `buildConciergeItems` 순수 규칙 엔진(React 밖·now 주입) — pre: urgent/체크인/백업/패킹/공항/레디니스(IDB 지도타일·문서금고·환율캐시 직접 확인) · during: 이동 카운트다운+공항 여유·체크아웃·예약 식당·내일 브리프·urgent 데드라인·출발일 아침 최종 레디니스·저녁 빈 기록 넛지 · 자급 날씨(필요 도시만 fetch, savedAt 불변) · **빈 날은 미렌더**. node 단위테스트 **46/46** |
| **기록 경험(30차)** | 저녁 한 줄 캡처(EveningReminder에 NoteEditor 인라인, 이미 적은 날 '이어 쓰기' 축약) · 기록 캘린더(기록 탭 27칸 — 사진=도시색·메모=골드 점·오늘 테두리) · 27일 잔재 정리(/28·07.12·PIN 카피 → 0) |
...
| **(20차) 주연용 온보딩** | `cfd2e07` | 첫 실행 1회 환영 하단시트(5탭 안내·둘이 연결·데이터 안심)·`onboarded` 플래그·`Onboarding` 컴포넌트 |
| **(21차) 여정 탭 3그룹** | `cbcff35` | 12 서브탭 평면→이동·숙박/돈·예산/현지 생활 3그룹+eyebrow(기능 불변, 위계·스캔성↑) |
| **(22차) 기기 연동 UX** | `9ac8576` | SyncPanel '기기 연동' 재프레이밍·3-step 안내·헤더 상시 동기화 배지(online/offline) |
| **(23차) 오늘의 현지 도구** | `330439a` | during 홈 `LocalToolkit` 2×2(통화·꿀팁·교통·복장/날씨)+여정 딥링크·비상연락처 오늘 위치 우선 |
| **(24차) 네이티브 뒤로가기** | `4d985f7` | `useBackClose`/`useScrollLock` — 모달·서브뷰 LIFO 백 닫기·스크롤 락·서브탭 위치 기억(iOS standalone 제외) |
| **(25차) 여정 그리드·자간 토큰** | `1e26d04`·`a943528` | 3그룹→4열×3행 아이콘 그리드 한 판·지도 핀 국기 제거·호텔 컴팩트 1행 + 자간 px 35곳 토큰화(픽셀 동일) |
| **(26차) Today 카드 리디자인** | `dd5c9f3` | 행동 우선 재배치(정체성→타임라인→할일→사진/지도→사색)·지금포커스/타임라인 중복 단일화(−41줄)·`QA_CHECKLIST.md` 신설 |
| **(27차) GT 컨시어지** | `c84ad6d` | **규칙 기반 '지금 필요한 것' 브리핑**(홈 최상단) — `buildConciergeItems` 순수 엔진+`ConciergeCard`·날조 0·빈 날 미렌더·단위테스트 23/23 |
| **(28차) 컨시어지 2단계** | `c32e23c` | **레디니스 감지**(IDB 지도타일 n/9·문서금고 0건·환율캐시 — 직접 확인, 실패 시 생략)·여행 중 urgent 데드라인(ZTL)·**자급 날씨**(필요 도시만, savedAt 불변)·테스트 33/33 |
| **(29차) 여행 중 실사용 마감** | `49698f1` | 가계부 통화 오늘 나라 자동(GBP/AED)·출발일 아침 최종 레디니스('마지막 와이파이')·항공 leg 공항 여유 힌트·테스트 40/40 |
| **(30차) 영원의 기록** | `4cc0041` | **저녁 한 줄 캡처**(질문 본 자리에서 NoteEditor 인라인)·**빈 기록 넛지**(사진·메모 둘 다 없을 때만)·**기록 캘린더**(27칸 도시색 채움)·27일 잔재 정리(/28·07.12·PIN 카피)·테스트 46/46 |
...
### 🎯 다음 개선 — 통합 우선순위 (2026-06-10 갱신 · 29차)

> **26~29차 누적 완료(코드)**: Today 카드 행동 우선 리디자인 · **GT 컨시어지 1~2단계**(규칙 엔진·레디니스·자급 날씨 — P2 "데일리 지금 포커스"를 종합 브리핑으로 대체 발전) · 가계부 스마트 통화 · 출발일 아침 최종 레디니스. **남은 🔴는 여전히 Tyler 직접(실데이터 편명 2건·실기기 QA = `QA_CHECKLIST.md`)**, 코드 신규 작업은 동결 원칙상 보류가 기본.
