...
> 정리일: **2026-05-31** (8차 갱신) · 정리자: Claude (Opus 4.8)
> 라이브: https://grandtour-lilac.vercel.app/  · Publish→Fork 씨앗(`9b1b821`) · 공동작업 실시간+초대 링크+swipe 수정기록(`9e4a6c1`) · **일정 사진 28일 전부 채움·정본화(트레비·판테온 실사·7/12 서울 일출), 전 작업 push 완료**
...
### 🎯 다음 개선 — 통합 우선순위 (2026-05-31 기준)

> 기준: **출발 중 5초 효용 + 실패 시 타격 크기.** 시각·콘텐츠 다듬기 < 출발 전 실패 방지. 분류: 🔴 출발 전 필수 / 🟡 체감 품질 / 🟢 여유·확장(동결 후).
> 이번 세션 누적 완료: 인터랙티브 지도·도시 몰입 히어로 · Publish→Fork 씨앗 · 공동작업 실시간+초대 링크+swipe 수정기록 · 일정 사진 전수 정본화.

| 순위 | 항목 | 분류 | 비고 |
|---|---|---|---|
| 1 | **실데이터 입력** | 🔴 | 호텔·항공 확정 예약번호, 미식 예약 상태, 가계부 통화·초기 예산. 비면 멋진 껍데기. **코드 아닌 입력 — Tyler 직접, 최우선.** |
| 2 | **실기기 아이폰 QA(누적분)** | 🔴 | 이번 세션 전부를 실제 폰 PWA에서 1회: 지도 핀치/팬·기록 공개·**공동작업 두 기기 실시간**·**swipe "누가 언제"**·`?trip=`/`?j=` 링크·SW 갱신. 실시간/제스처는 헤드리스로 못 본 부분 — 실측 필수. |
| 3 | **오프라인 지도 타일 선캐싱 + 문서 금고** | 🔴 | 로밍 끊겨도 도시 지도 그림(IDB 선캐싱) · 여권·보험·바우처 PIN+IDB 오프라인 저장. 분실·입국심사 대비. |
| 4 | **데이터 복원력(출발 직전 백업)** | 🔴 | 메모는 공동작업 시 클라우드 사본 생기나 **사진은 단일 기기**. 출발 직전 사진 포함 전체 백업 강제 리마인더. |
| ~~5~~ | ~~q·mo 톤 점검~~ ✅ | — | **완료** — 28개 `q` 전수 공동·보편+친근 해체체로 통일(개인 커리어 표현 D7·15·25·10 제거). `mo`는 이미 공동·따뜻한 톤이라 불변. 잔여 `회사`(동인도회사)·`청사진`(챕터 은유/관점)은 역사·은유라 의도 유지. |
| 6 | **공동작업 디테일** | 🟡 | swipe 발견성 힌트(첫 사용 시 "←밀어 보기")·데스크톱 어포던스 · 메모 동시편집 LWW 충돌 인디케이터/경고 · 관점도 swipe 메타 확장. |
| 7 | **첫 로드 성능(Lighthouse)** | 🟡 | Babel 런타임 컴파일(파일 ~430KB) → critical CSS 인라인·폰트 preload·로딩 스켈레톤으로 체감 개선. |
| 8 | **접근성(a11y)** | 🟡 | 터치 44px·`aria-label`·focus-visible·prefers-reduced-motion 점검. |
| 9 | **"여정의 GitHub" 비전 확장** | 🟢 | 커밋 타임라인(과정 기록)·fork→원작 upstream PR·디스커버리·카운터 신뢰화. 동결 후/귀국 후. |
