...
> 정리일: **2026-07-15** (47차 갱신) · 정리자: Claude (Fable 5)
...
> **여행 완료: 2026-06-15 → 07-11 (27일)** · 앱은 **post(회고) 모드** — 이후 우선순위는 출발 준비가 아니라 **회고·아카이브·기록 보존**.
> 푸시 상태: **`origin/main`==`d5d04b1` — 45차 docs+46·47차 4커밋 푸시(2026-07-15, Tyler 명시 요청)** · Vercel 자동 배포.
>
> **46~47차 요약(코드)**: 46차 **회고록 내보내기**(`buildMemoirHtml` — 사진 base64 임베드·메모 전문·관점·가계부 결산을 담은 **자립형 HTML 한 파일**, 인쇄 CSS→PDF, 날조 0·전문 이스케이프)(`20f2aec`) · 47차 **post UX 고도화(매일 여는 앱)** — 홈 '추억 우선' 재배치(오늘의 추억 최상단→빈 기록 넛지→콤팩트 히어로→🗂 회고 보관함 `RetroArchiveCard`)+**📸 앨범 탭**(post: 하단 '체크'→'앨범' `AlbumView`, 27일 그리드→그날 펼침)+**goToDay 죽은 daysFocus 버그 수정**(pendingDayFocus 일원화)+시제·카피 12건(D+N 헤더·백업 post 기본 ON/문구·"9개 도시의 기억"[점=기록 유무]·관점 티어 한국어화·완성 취소선 제거·메모 첫 줄 발췌 등)(`3fbc443`).
>
> **31~45차 요약(코드)**: 31차 글자 긁힘(래스터화 아티팩트) 원인 5종 수술(`45677b5`) · 32차 느린 네트워크 3.5s 즉시 부팅+일정/도시 사진 오프라인 캐시+**SW v6**(`b5b4072`) · 33차 PC↔폰 첫 연결 `pullAll` 초기 시딩(`621f188`) · 34차 날씨 '우리가 있는 날' 체류일 강조(`f66c3fe`) · 35차 예약 링크 전수+29곳 사용 가이드(`23ea4a2`) · 36차 실편명 2건+**회고 리플레이 `ReplayOverlay`**(`09bbf71`) · 37차 **Day 오프셋 버그 수정(ceil→캘린더 일수, 6/15=Day1)**+during 히어로 대시보드(`9a0aa38`) · 38차 히어로 하루 플랜+**부모님 관전 읽기전용 `?view`**(`24bdd34`) · 39차 하루 플랜 가로 진행 바(`ff7af81`) · 40차 히어로↔Today 중복 제거+공유 편집/관전 2그룹(`9a9eacf`) · 41차 날씨 라벨 role 기반 정확화(`63f8f5b`) · 42차 공유 링크 항상 Vercel 주소 강제(`b411a22`) · 43차 사진 갤러리 선택+메모 auto-grow+**사진 PC↔폰 동기화(`photo_<n>` 행 LWW)**(`2cbf059`) · 44차 관점 auto-grow+**도시별 관점 렌즈 `CITY_LENS`**(`c91b5aa`) · (무차수 06-23) pullAll LWW 타임스탬프 가드+**오프라인 쓰기 큐**(`fb904d8`·`83c04b9`) · **45차 회고(post) 모드 디벨롭 — post 홈 회고 동선 재구성+🧾 가계부 결산+📸 오늘의 추억 한 조각+▶ 리플레이 자동재생·스와이프+✍️ 빈 기록 넛지**(`1a2a870`·`2373039`).
>
...
| 지표 | 값 |
|---|---|
| **index.html** | 9,709줄 · ~596KB (실제 지리 베이스맵 path 포함) · top-level function 80 |
| **React 컴포넌트** | 27차 `ConciergeCard` · 36차 `ReplayOverlay` · 44차 `CityLensSection`/`OutputNote` · 45차 `MemoryOfToday` · 47차 `AlbumView`/`RetroArchiveCard` 등 (App 포함, 전부 함수형) |
| **회고(post) 모드(45~47차)** | post 홈 = **'추억 우선' 동선(47차)**: 📸 오늘의 추억(`MemoryOfToday`, 결정적 일일 회전) → ✍️ 빈 기록 넛지 → 콤팩트 히어로(3-stat·▶리플레이 자동재생/스와이프) → 🗂 **회고 보관함**(`RetroArchiveCard` — ⭐평생 관점·🧾가계부 결산·📕**회고록 내보내기**[46차 `buildMemoirHtml`, 자립형 HTML·인쇄→PDF]). 하단 탭 post: '체크'→**📸 앨범**(`AlbumView` 27일 그리드→그날 펼침, 체크리스트는 하단 링크). 백업 사진 포함 post 기본 ON. 넛지·결산·추억 전부 **날조 0**(데이터 없으면 미렌더) |
...
| **Service Worker** | `grandtour-v6`(32차 — 느린 네트워크 3.5s 부팅·사진 오프라인 캐시) · push/notificationclick 핸들러 |
...
| **(31차) 글자 긁힘 수술** | `45677b5` | 스크롤·전환 중 텍스트 래스터화 아티팩트 원인 5종 제거 |
| **(32차) 오프라인 연속성** | `b5b4072` | 느린 네트워크 3.5s 즉시 부팅(**SW v6**)·일정/도시 사진 오프라인 캐시·카드 press 통일 |
| **(33차) PC 연동 시딩** | `621f188` | 첫 연결 `pullAll` — 원격 우선 pull, 원격에 없는 키만 로컬 seed |
| **(34차) 날씨 체류일 강조** | `f66c3fe` | STAY_DATES 체류일 스트립+행 도시색 하이라이트+여행중 오늘 도시 기본 선택 |
| **(35차) 예약 링크 점검** | `23ea4a2` | 링크 전수(보험 URL 인코딩·보르게세 도메인)+예약 29곳 '들어가면 어떻게' 한 줄 가이드 |
| **(36차) 회고 리플레이** | `09bbf71` | 실편명 2건(A3 7301·EY 90)+**`ReplayOverlay` '27일을 한 편으로'**(post 전용) |
| **(37차) Day 버그+대시보드** | `9a0aa38` | **Day 오프셋 수정(ceil→캘린더 일수, 6/15=Day1)**+여행 중 히어로 대시보드(Day N·현재/다음 도시 날씨) |
| **(38차) 하루 플랜+부모님 관전** | `24bdd34` | 히어로 하루 플랜+**읽기전용 `?view` 링크**(VIEW_ONLY push 전면 차단·outputs 동기화 합류·ViewerBanner) |
| **(39차) 가로 진행 바** | `ff7af81` | 하루 플랜 세로 리스트→가로 진행 바(현재 시각 마커+'지금쯤' 활동) |
| **(40차) 중복 제거·공유 정리** | `9a9eacf` | Today 카드 히어로 중복(정체성·타임라인) 삭제+SyncPanel 편집/관전 2그룹 |
| **(41차) 날씨 라벨 정확화** | `63f8f5b` | wxCities `role` 기반 — 비행/서막일 다음 도시='도착지 날씨'(오기 수정) |
| **(42차) 공유 링크 강제** | `b411a22` | localhost/file 미리보기에서 복사해도 **항상 Vercel 배포 URL** 생성+'기기 꺼져도 열림' 안내 |
| **(43차) 사진 동기화** | `2cbf059` | 갤러리 선택(capture 제거)·메모 auto-grow·**사진 PC↔폰 `photo_<n>` 행 개별 upsert+savedAt LWW**(관전 제외), 테스트 12/12 |
| **(44차) 관점 렌즈** | `c91b5aa` | 관점 칸 auto-grow(`OutputNote`)+**`CITY_LENS` 9도시 경제·역사·정치**+동기화 함정 경고(양쪽 코드 생성) |
| **(무차수 06-23) 오프라인 큐** | `fb904d8`·`83c04b9` | pullAll LWW 타임스탬프 가드+**오프라인 쓰기 큐**(미접속 중 기록 유실 방지) |
| **(45차) 회고(post) 모드** | `1a2a870`·`2373039` | **post 홈 회고 동선 재구성**(체크 StatCard·비상연락처 정리, 빠른 이동→기록·가계부)+**🧾 가계부 결산**+**📸 오늘의 추억 한 조각**(`MemoryOfToday`, 결정적 일일 회전)+**▶ 리플레이 자동재생·스와이프**+**✍️ 빈 기록 넛지**. 테스트 13/13+19/19 |
| **(46차) 회고록 내보내기** | `20f2aec` | **자립형 HTML 회고록 한 파일**(`buildMemoirHtml`) — 챕터별 27일(사진 base64 임베드·메모 전문)+관점+가계부 결산, 인쇄 CSS→PDF. RetrospectHero 📕 버튼(백업 동일 Blob 패턴). 날조 0·전문 이스케이프. 테스트 31/31 |
| **(47차) post UX 고도화** | `3fbc443` | **추억 우선 홈**(오늘의 추억 최상단→넛지→콤팩트 히어로→🗂 회고 보관함)+**📸 앨범 탭**(post: 체크→앨범, 27일 그리드→그날 펼침)+**goToDay 죽은 daysFocus 버그 수정**(pendingDayFocus 일원화)+**시제·카피 12건**(D+N·백업 post 기본 ON/문구·9개 도시의 기억[점=기록 유무]·티어 한국어화·취소선 제거·메모 첫 줄 발췌 등). 마커 24/24·memoir 회귀 31/31 |
...
### 🧭 여행 종료 후 우선순위 (2026-07-14 · 45차 갱신 — 이 표가 현행)

> **여행 완료(6/15~7/11)로 아래 P1(출발 전 필수)은 소멸.** 렌즈 교체: "여행 중 5초 효용 + 실패 시 타격" → **"기록이 평생 남는가 + 다시 열게 되는가"**. 45차에서 post 홈이 회고 동선으로 재구성됨(추억 한 조각·리플레이 자동재생·가계부 결산·빈 기록 넛지).

| 순위 | 항목 | 분류 | 비고 |
|---|---|---|---|
| 1 | **기록 영구 보존 — 사진 포함 전체 백업** | 🔴 | 여행 기록(사진·메모·관점)은 유일본. 홈 백업 카드에서 사진 포함 ON 1회 — **Tyler 직접.** |
| 2 | **빈 기록 채우기** | 🔴 | 기억이 생생할 때 — 45차 넛지·추억 카드가 매일 유도. 앱 내 입력, **Tyler·주연 직접.** |
| 3 | **폰 실측(45~47차 분)** | 🟡 | post 홈 — 추억 우선 동선·앨범 탭·회고 보관함·D+N 헤더·백업 사진 포함 기본 ON·리플레이 자동재생/스와이프·회고록 다운로드(인쇄→PDF 포함). |
| 4 | **사진 관전 공유(부모님 `?view`)** | 🟡 | 38차 결정(관전자 사진 제외)의 **명시적 뒤집기**가 필요 — 사용자 결정 대기. `gt_state` `photo_<n>` 행 재활용으로 코드 부담 낮음(43차 인프라). |
| 5 | ~~**회고 내보내기**~~ ✅ 1차 완료(46차) | 🟢 | **자립형 HTML 회고록**(`buildMemoirHtml`+RetrospectHero 📕 버튼, `20f2aec`) — 사진 base64·메모 전문·관점·가계부 결산 한 파일, 브라우저 인쇄→PDF. 잔여는 슬라이드/영상 형태(선택). |

### 🔴 P1 — 출발 전 필수 (해외 실패 방지 + 실데이터) · ~~D−12~~ **여행 종료로 소멸(기록용)**
...
| 항목 | 임팩트 | 비고 |
|---|---|---|
| ~~사진·메모 동기화 확장~~ ✅ | — | **완료** — 메모는 7차(`notes` SYNC_KEYS 합류), 사진은 43차(`photo_<n>` 행 개별 upsert+LWW, `2cbf059`) + 오프라인 쓰기 큐(`83c04b9`). bar #2(무손실)·#6(공유) 🟢. 잔여는 관전자(부모님) 사진 공유뿐(위 표 #4). |
| **사진 → 여정 자동 정렬** | 중간 | 촬영 시각/위치 기반 Day 자동 매칭·지도 핀. 회고(RetrospectHero) 품질 급상승. |
| **PWA 백그라운드 푸시** | 중간 | 진짜 저녁 회고 알림. VAPID 키 + 서버 필요(현 SW 핸들러는 준비됨). 여행 종료로 우선순위 하락. |
| **갤러리 영상 + 회고 내보내기** | 중간↑ | 회고 리플레이는 앱 내 완성(36차 `ReplayOverlay`+45차 자동재생). 남은 건 **내보내기**(PDF/슬라이드/영상) — 여행 종료로 P3 중 체감 최상. IDB 용량 검토. |
...
- **2026-07-14 현재 `main` == `origin/main` == `2373039`** — 31~45차 전부 푸시·라이브(45차 2커밋은 명시 요청 푸시 후 라이브 마커 검증, 06-23 오프라인 큐 2건 포함). working tree clean(본 REVIEW 45차 갱신만 커밋 대기). **유실 작업 없음.**
- **2026-07-15 현재 `main` == `origin/main` == `d5d04b1`** — 45차 docs(`ad190c9`)+46차(`20f2aec`·`c49803c`)+47차(`3fbc443`·`d5d04b1`) 5커밋 명시 요청 푸시. **유실 작업 없음.**
