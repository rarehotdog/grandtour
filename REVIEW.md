...
> 정리일: **2026-07-14** (45차 갱신) · 정리자: Claude (Fable 5)
...
> 푸시 상태: **`origin/main`==`2373039` — 45차 2커밋 푸시·라이브 마커 검증 완료(2026-07-14, Tyler 명시 요청)** · Vercel 자동 배포.
>
...
| 지표 | 값 |
|---|---|
| **index.html** | 9,447줄 · ~580KB (실제 지리 베이스맵 path 포함) · top-level function 77 |
| **React 컴포넌트** | 27차 `ConciergeCard` · 36차 `ReplayOverlay` · 44차 `CityLensSection`/`OutputNote` · 45차 `MemoryOfToday` 등 (App 포함, 전부 함수형) |
| **회고(post) 모드(45차)** | post 홈 = RetrospectHero(3-stat·⭐평생 관점·**🧾 가계부 결산**·**✍️ 빈 기록 넛지**) + **📸 오늘의 추억 한 조각**(`MemoryOfToday` — 로컬 자정 기준 결정적 일일 회전, 사진+메모 발췌, 그날 일정/메모 딥링크) + **▶ 리플레이 자동재생(5초/장·수동 개입 시 일시정지·⏸/▶)·좌우 스와이프**. 준비용 위젯(체크 StatCard·비상연락처·빠른 이동 체크/패킹)은 post에서 숨김/교체. 넛지·결산·추억 전부 **날조 0**(데이터 없으면 미렌더) |
...
| 3 | **폰 실측(45차 분)** | 🟡 | post 홈 — 추억 카드 일일 회전·리플레이 자동재생/스와이프·가계부 결산 금액·빠른 이동 회고형. |
