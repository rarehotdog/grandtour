...
> 정리일: **2026-06-10** (29차 갱신) · 정리자: Claude (Fable 5)
...
> 푸시 상태: `origin/main`==`a943528`(자간 토큰화까지 푸시 완료). 로컬 **10커밋 미푸시**(25~29차) — **사용자 푸시 보류 지시**, "푸시해줘" 시 푸시.
>
> **25~29차 요약(코드)**: 25차 여정 탭 3그룹 → 12 아이콘 그리드·지도 핀 국기 제거·호텔 컴팩트 행(`1e26d04`)+자간 px 토큰화(`a943528`) · 26차 Today 카드 행동 우선 재배치+지금포커스/타임라인 중복 제거(`dd5c9f3`)+`QA_CHECKLIST.md` 신설 · **27차 GT 컨시어지 — 규칙 기반 '지금 필요한 것' 브리핑(홈 최상단, buildConciergeItems 순수 엔진+ConciergeCard)**(`c84ad6d`) · **28차 컨시어지 2단계 — 레디니스 감지(IDB 지도타일·문서금고·환율캐시)+여행 중 urgent 데드라인+자급 날씨**(`c32e23c`) · 29차 가계부 스마트 통화(오늘 나라 통화 자동)+출발일 아침 마지막 레디니스+공항 여유 힌트(`49698f1`).
>
...
| 지표 | 값 |
|---|---|
| **index.html** | 8,694줄 · ~490KB (실제 지리 베이스맵 path 포함) · top-level function 72 |
| **React 컴포넌트** | 64개 top-level + 인라인 (App 포함, 함수형). 27차 신규 `ConciergeCard` |
| **GT 컨시어지(27~29차)** | `buildConciergeItems` 순수 규칙 엔진(React 밖·now 주입) — pre: urgent/체크인/백업/패킹/공항/레디니스(IDB 지도타일·문서금고·환율캐시 직접 확인) · during: 이동 카운트다운+공항 여유·체크아웃·예약 식당·내일 브리프·urgent 데드라인·출발일 아침 최종 레디니스 · 자급 날씨(필요 도시만 fetch, savedAt 불변) · **빈 날은 미렌더**. node 단위테스트 **40/40** |
