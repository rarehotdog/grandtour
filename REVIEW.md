...
> 정리일: **2026-06-09** (22차 갱신) · 정리자: Claude (Opus 4.8)
...
> 푸시 상태: `origin/main`==`2617430`(19차 docs까지 푸시 완료). 로컬 **7커밋 미푸시**(`1ed8137` 빈상태·`801edae` docs·`cfd2e07` 온보딩·`cbcff35` 여정 그룹화·`65fc142` 20·21차 docs·`9ac8576` 기기연동UX+헤더배지 + 본 SESSION_LOG/REVIEW 갱신) — Tyler 수동 푸시 대기.
>
> **20~22차 요약(코드)**: 20차 주연용 첫 실행 온보딩 오버레이(`cfd2e07`) · 21차 여정 12서브탭 → 이동·숙박/돈·예산/현지생활 3그룹 재구성(`cbcff35`) · **22차 기기 연동 UX 재정렬(친구 초대 → 본인 다기기 '🔗 기기 연동', 내 이름 상시 노출·3-step 안내) + 헤더 상시 동기화 배지(online/offline·탭→홈)**(`9ac8576`, 메커니즘 불변).
>
...
| 지표 | 값 |
|---|---|
| **index.html** | 8,374줄 · ~470KB (실제 지리 베이스맵 path 포함) · top-level function 68 |
