...
> 정리일: **2026-06-02** (9차 갱신) · 정리자: Claude (Opus 4.8)
> 라이브: https://grandtour-lilac.vercel.app/  · **오프라인 지도 타일 선캐싱·문서 금고·출발 전 전체 백업 리마인더**(`cef1613`) · 본문 16px·자간 토큰화·일정 행별 구글맵 링크(`9b4a1e1`) · 성능(preconnect)·접근성·**체크리스트 카테고리 완료율**(`6eb9f3d`) · 일정↔추천 연결·데일리 "지금" 포커스(`f0e0548`). **전 작업 push 완료(`f0e0548`)**
> 저장소: https://github.com/rarehotdog/grandtour
> 출발: **2026-06-15** (D−13) · 코드 동결 권장: **2026-06-08** (D−6)
...
| 지표 | 값 |
|---|---|
| **index.html** | 7,578줄 · ~450KB (실제 지리 베이스맵 path 포함) |
| **React 컴포넌트** | 67개 (App 포함, 함수형) |
...
| **체크리스트** | 카테고리별 진행바·🔴 urgent 미완료 배지·완료 자동 하단 정렬 |
...
### 🎯 다음 개선 — 통합 우선순위 (2026-06-02 기준 · 9차)

> 기준: **출발 중 5초 효용 + 실패 시 타격 크기.** 시각·콘텐츠 다듬기 < 출발 전 실패 방지. 분류: 🔴 출발 전 필수 / 🟡 체감 품질 / 🟢 여유·확장(동결 후).
> **9차 누적 완료(코드)**: 오프라인 지도 타일 선캐싱 · 문서 금고 · 출발 전 전체 백업 강조 · 첫 로드 preconnect · 접근성 보강 · 체크리스트 카테고리 완료율 · 일정↔추천 연결 · 데일리 "지금" 포커스 · 본문 16px·자간 토큰화·일정 맵링크. **→ 코드로 할 수 있는 🔴 출발 전 항목은 일단락. 남은 🔴는 Tyler 직접 작업.**
...
| 6 | **공동작업 디테일** | 🟡 | swipe 발견성 힌트("←밀어 보기")·데스크톱 어포던스 · 메모 동시편집 LWW 충돌 인디케이터 · 관점도 swipe 메타 확장. |
| ~~7~~ | ~~첫 로드 성능~~ 🟡 부분완료 | — | **preconnect 적용(`6eb9f3d`)**. Babel 런타임 컴파일(~450KB)이 근본 병목 — 더 큰 개선은 빌드 도입 영역(단일파일 철학상 ❌). 로딩 스플래시는 이미 존재. |
...
- **2026-06-02 현재 `main` == `origin/main` == `f0e0548`** (9차 4커밋 `9b4a1e1`→`cef1613`→`6eb9f3d`→`f0e0548` push 성공·Vercel 배포, working tree에 본 REVIEW.md 갱신만 미커밋).
- 9차 선형 적층: `d96e89f → 9b4a1e1 → cef1613 → 6eb9f3d → f0e0548`. **유실 작업 없음.**
- 평소엔 Tyler 수동 푸시(사내망 차단)지만, 9차도 사용자 명시 요청으로 Claude가 커밋·푸시 수행(`9b4a1e1`·`f0e0548` 2회)했고 정상 푸시됨. 기본 규칙(요청 시에만 푸시)은 유지.
