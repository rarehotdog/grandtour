...
새 채팅 시작 → 다음 첨부:
1. `CLAUDE.md` (이 문서)
2. `index.html` (소스 오브 트루스 — 단일 파일)

> ⚠️ `companion_app.jsx`, `build_html.py` 는 **[DEPRECATED]** — 더 이상 사용하지 않음.
> `index.html`이 유일한 소스 오브 트루스 (Source of Truth).
...
```
grandtour/
├── index.html              # ← Source of Truth (유일한 소스, Vercel이 서빙)
├── sw.js                   # Service Worker (PWA 오프라인 지원)
├── manifest.webmanifest    # PWA 매니페스트 (홈 화면 추가)
├── vercel.json             # Vercel 설정 (캐시 헤더)
├── README.md               # 짧은 안내
├── CLAUDE.md               # ← 이 문서
├── QUICK_START.md          # 빠른 시작 가이드
├── REVIEW.md               # 코드 리뷰 노트
├── companion_app.jsx       # [DEPRECATED] 구버전 JSX 소스 — 사용 안 함
└── build_html.py           # [DEPRECATED] 빌드 스크립트 — 사용 안 함
```

### 빌드 흐름

```
index.html (직접 수정)  ──git push──▶  Vercel (30초 자동 배포)
```

**빌드 단계 없음.** `index.html` 을 직접 수정하고 git push 하면 끝.
단일 파일 철학 = 의존성 0 = 복잡도 0.

> ~~`companion_app.jsx` → `build_html.py` → `index.html`~~ (구 플로우, 폐기됨)
...
- **Service Worker** (PWA 오프라인 캐싱 — sw.js)
- **빌드 도구 없음** — 단일 HTML 파일
...
| 3370-3384 | ErrorBoundary + render + SW 등록 | 앱 진입점 |
...
- **Phase 0 결정**: `companion_app.jsx` + `build_html.py` 빌드 파이프라인 폐기. `index.html` 단독 편집으로 단순화.
...
```
1. [읽기] index.html 의 변경 대상 섹션 view (코드 맵 라인 범위 참조)
2. [수정] str_replace 또는 Edit 도구로 직접 편집
3. [확인] 로컬에서 open index.html 또는 더블클릭
4. [푸시] git add . && git commit -m "..." && git push
5. [대기] 30초 후 https://grandtour-lilac.vercel.app/ 자동 배포
```

> ~~`python3 build_html.py`~~ 빌드 단계 없음. 직접 편집 후 바로 push.
...
```bash
# 1. HTML 줄 수 확인 (예상치 안에 있는지)
wc -l index.html

# 2. 컴포넌트 함수 갯수 확인 (현재 30+개)
grep -c "^function " index.html

# 3. ReactDOM 렌더 진입점 확인
grep "ReactDOM.createRoot" index.html

# 4. 라우팅 무결성 확인
grep "view ===" index.html | head

# 5. ErrorBoundary + SW 등록 확인
grep -n "ErrorBoundary\|serviceWorker.register" index.html
```
...
```bash
# index.html에서 HOTELS_DEFAULT 찾기
grep -n "const HOTELS_DEFAULT" index.html
# → 307 근처
```
...
`index.html` 안에서 `storage.get/set` (비동기 래퍼) 또는 `localStorage` 직접 호출 모두 가능.
`storage` 래퍼는 index.html 내부에 정의된 헬퍼 함수임 (별도 빌드 변환 없음).
...
### 8. **companion_app.jsx / build_html.py**
`grandtour-old/` 폴더에 백업 보존. 절대 `grandtour/` 메인 폴더로 복원하지 말 것.
`index.html`이 유일한 소스 오브 트루스.
...
### 우선순위 ★
- [x] **PWA 매니페스트 + 오프라인 모드 (Service Worker)** — Phase 0 완료 (sw.js + manifest.webmanifest)
...
- `companion_app.jsx` + `build_html.py` 파이프라인 부활 — 폐기 결정 확정
...
좋은 요청:
> "index.html의 RestaurantTracker 컴포넌트에 메모 필드 추가해줘. 각 레스토랑마다 자유 메모 적을 수 있게. localStorage 키는 `restaurant_notes` 신규."

너무 모호한 요청:
> "레스토랑 좀 더 좋게 만들어줘"

작업 결과물 요청 시 항상:
1. `index.html` (수정된 파일 — 유일한 소스 오브 트루스)
2. 변경 요약 (어떤 라인, 무슨 이유)
...
| 날짜 | 버전 | 내용 |
|------|------|------|
| 2026-05-27 | Phase 0 | Source of Truth = index.html 확정. companion_app.jsx + build_html.py deprecated. sw.js + manifest.webmanifest + ErrorBoundary + safe-area 추가. Git 복구 (grandtour-old 백업). |
